	...
	<div class="flex pb-52">
		...
		<div class="max-md:w-full md:w-10/12">
			<div class="py-28 px-10">
				<p class="mb-5 text-lg font-semibold">Tambah Pengguna</p>
				<div
					class="card flex flex-wrap align-items-center justify-content-center gap-3 mb-5"
					v-if="$page.props.flash.message != null"
				>
					<Message severity="success" :closable="false"
						>Pengguna Berhasil Di tambahkan</Message
					>
				</div>
				<form @submit.prevent="form.post(route('addUserLoading'))">
					<div class="mb-5">
						<p>Nama Pengguna</p>
						<input
							type=""
					...
					<div class="flex pt-5">
						<Button label="Submit" outlined type="submit" />
					</div>
				</form>
			</div>
		</div>
...
function image(e) {
	...
	if (files.value.size > 6500000) {
		return (message.value = "File tidak boleh lebih dari 6.5MB");
	}
